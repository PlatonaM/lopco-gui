...
return null;}
const url=new URL(window.location);const route=getRoute(url.pathname);let main_element=document.getElementsByTagName("main")[0];if(route){document.addEventListener('DOMContentLoaded',(event)=>{import('../components/'+route[0][1]+'/component.min.js').then((cmp)=>{window.active_cmp=new cmp[route[0][2]](main_element);window.active_cmp.draw((url.search)?url.searchParams:undefined,route[1]);}).catch((err)=>{main_element.innerHTML=err;})});}else{if(!(url.pathname==='/')){main_element.innerHTML='Page not found.';}}