...
export{Form}
class Form{constructor(ctr){this.container=ctr;this.conf_count=0;}
...
addConfigFields(element){fetch('/components/workers/form/conf-fields-template.html').then((response)=>response.text()).then((template)=>{let parent=element.parentElement;if(parent.childElementCount<2){element.className+=" uk-margin-bottom";}
parent.append(document.createRange().createContextualFragment(Mustache.render(template,{id:this.conf_count})));this.conf_count++;});}
