...
submit(event){event.preventDefault();const form=new FormData(event.target);fetch(active_cmp.constructor.api+'/'+form.get('form-id'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:form.get('form-name'),pipeline_id:form.get('form-pipeline'),type_id:form.get('form-type')}),}).then(response=>{if(response.ok){alert('Data-Source saved successfully!');window.open('/data-sources','_self');}else{throw response.status;}}).catch((error)=>{alert("Can't save Data-Source: "+error);});}}