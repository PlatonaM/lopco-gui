...
draw(ds_data=null){fetch(DSForm.p_api).then((response)=>{if(response.ok){return response.json()}
throw'Error retrieving pipelines - '+response.status;}).then((data)=>{fetch('/components/data-source-form/template.html').then((response)=>response.text()).then((template)=>{let items=[];for(let[key,value]of Object.entries(data)){value=JSON.parse(value);items.push({p_id:key,p_name:value['name'],selected:(ds_data)?(key===ds_data['pipeline_id']):false})}
...
submit(event){event.preventDefault();const form=new FormData(event.target);fetch(DSForm.api+'/'+form.get('form-id'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:form.get('form-name'),pipeline_id:form.get('form-pipeline'),type_id:form.get('form-type')}),}).then(response=>response.text()).then((data)=>{alert('Data-Source saved successfully!');window.open('/data-sources/registry','_self');}).catch((error)=>{alert("Can't save Data-Source: "+error);});}}
class DSFormEdit extends DSForm{constructor(ctr){super(ctr);}
draw(sp,{ds}){fetch(DSForm.api+'/'+ds).then((response)=>{if(response.ok){return response.json()}
throw'Error retrieving Data-Source - '+response.status;}).then((data)=>{data['id']=ds;super.draw(data);}).catch((err)=>{this.container.innerHTML=err;});}}