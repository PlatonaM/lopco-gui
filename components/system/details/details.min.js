...
throw'Error retrieving Protocol-Adapter - '+response.status;}).then((data)=>{fetch('/components/system/details/template.html').then((response)=>response.text()).then((template)=>{let ports=[];if(data['ports']){const entries=Object.entries(data['ports']);let e_length=entries.length;for(let[port,host]of entries){ports.push({port:port,host_i:(host)?host['host_interface']:'null',host_p:(host)?(e_length>1)?host['host_ports'].toString()+'; ':host['host_ports'].toString():'null'});e_length--;}}
this.container.innerHTML=Mustache.render(template,{name:srv,image:data['image']['name'],hash:data['image']['hash'],status:data['status'],ports:ports,type:data['labels']['lopco-type']});let log_container=document.getElementById('log-ctr');this.getLog(srv,log_container,true);window.setInterval(this.getLog,5000,srv,log_container);});}).catch((err)=>{this.container.innerHTML=err});}
