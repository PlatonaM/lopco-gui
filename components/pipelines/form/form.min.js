...
addInitStage(){const st_id=this.genRandomID();this.stage_container.append(document.createRange().createContextualFragment(Mustache.render(Form.stage_template,{init:true,name:'init',id:st_id,output:[{name:'init_source',media_type:'',is_file:true}]})));this.stages[Object.keys(this.stages).length]={id:st_id,wk_id:null};console.log(this.stages);}
...
removeStage(id){let element=document.getElementById(id);let parent=element.parentElement;parent.removeChild(element);for(let[key,value]of Object.entries(this.stages)){if(value['id']===id){delete this.stages[key]
break;}}}}