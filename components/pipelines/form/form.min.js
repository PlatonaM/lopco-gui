...
this.stage_container=document.getElementById('stages');this.addInitStage();});}).catch((err)=>{this.container.innerHTML=err;});}
...
if((Object.keys(this.stages).length>1)&&(st_num!==Object.keys(this.stages).length)){if(st_num-1===0){this.repopulateInputs(st_num,[{name:'init_source'}]);}else{if(this.workers[this.stages[st_num-1]['wk_id']]['output']){this.repopulateInputs(st_num,this.workers[this.stages[st_num-1]['wk_id']]['output']['fields']);}}}}
