...
addStage(wk_id,data=null){const st_num=Object.keys(this.stages).length;let inputs=[];let i_num=0;if(this.workers[wk_id]['input']){for(let item of this.workers[wk_id]['input']['fields']){inputs.push({name:item['name'],media_type:item['media_type'],is_file:item['is_file'],i_num:i_num});i_num++;}}
...
c_num++;}}
const st_id=this.genRandomID();this.stage_container.append(document.createRange().createContextualFragment(Mustache.render(Form.stage_template,{id:st_id,number:st_num,description:(data)?data['description']:null,w_name:this.workers[wk_id]['name'],w_id:wk_id,input:inputs,values:i_values,output:(this.workers[wk_id]['output'])?this.workers[wk_id]['output']['fields']:null,has_configs:!!(this.workers[wk_id]['configs']),configs:configs})));this.stages[st_num]={id:st_id,wk_id:wk_id};}
