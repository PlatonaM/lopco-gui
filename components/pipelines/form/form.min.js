...
this.stage_container=document.getElementById('stages');});}).catch((err)=>{this.container.innerHTML=err;});}
...
submit(event,method='POST'){event.preventDefault();const form=new FormData(event.target);for(let[key,value]of form.entries()){console.log(key+'='+value);}}
...
addStage(wk_id){const st_num=Object.keys(this.stages).length;let inputs=[];let i_num=0;if(this.workers[wk_id]['input']){for(let item of this.workers[wk_id]['input']['fields']){inputs.push({name:item['name'],media_type:item['media_type'],is_file:item['is_file'],i_num:i_num});i_num++;}}
let i_values=[];if(st_num===0){i_values.push({name:'init_source'})}else{if(this.workers[this.stages[st_num-1]['wk_id']]['output']){for(let item of this.workers[this.stages[st_num-1]['wk_id']]['output']['fields']){i_values.push(item);}}}
...
c_num++;}}
const st_id=this.genRandomID();this.stage_container.append(document.createRange().createContextualFragment(Mustache.render(Form.stage_template,{id:st_id,number:st_num,w_name:this.workers[wk_id]['name'],w_id:wk_id,input:inputs,values:i_values,output:(this.workers[wk_id]['output'])?this.workers[wk_id]['output']['fields']:null,has_configs:!!(this.workers[wk_id]['configs']),configs:configs})));this.stages[st_num]={id:st_id,wk_id:wk_id};}
...
if((Object.keys(this.stages).length>0)&&(st_num!==Object.keys(this.stages).length)){if(st_num===0){this.repopulateInputs(st_num,[{name:'init_source'}]);}else{if(this.workers[this.stages[st_num-1]['wk_id']]['output']){this.repopulateInputs(st_num,this.workers[this.stages[st_num-1]['wk_id']]['output']['fields']);}}}
for(let[key,value]of Object.entries(this.stages)){let num_e=document.getElementById(value['id']+'-number');num_e.value=key;}}
repopulateInputs(st_num,fields){let i;for(i=0;i<this.workers[this.stages[st_num]['wk_id']]['input']['fields'].length;i++){let in_elm=document.getElementById(this.stages[st_num]['id']+'-input-'+i+'-value');while(in_elm.options.length>1){in_elm.options.remove(in_elm.options.length-1);}
