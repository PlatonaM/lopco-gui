...
submit(event){event.preventDefault();const form=new FormData(event.target);fetch(active_cmp.constructor.mr_api+'/'+form.get('form-id'),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:form.get('form-name'),pipeline_id:form.get('form-pipeline'),type_id:form.get('form-type')}),}).then(response=>response.text()).then((data)=>{alert('Data-Source saved successfully!');window.open('/data-sources','_self');}).catch((error)=>{alert("Can't save Data-Source: "+error);});}}