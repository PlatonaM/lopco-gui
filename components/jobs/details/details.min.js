...
throw'Error retrieving job - '+response.status;}).then((data)=>{fetch('/components/jobs/details/template.html').then((response)=>response.text()).then((template)=>{let stages=[];let i;for(i=0;i<data['stages'].length;i++){const gen_fields=this.genFields(data['stages'][i]['outputs'])
