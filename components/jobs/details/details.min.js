...
throw'Error retrieving job - '+response.status;}).then((data)=>{fetch('/components/jobs/details/template.html').then((response)=>response.text()).then((template)=>{let stages=[];let i;for(i=0;i<Object.keys(data['stages']).length;i++){const inputs=this.genFields(data['stages'][i]['inputs']);console.log(inputs);const outputs=this.genFields(data['stages'][i]['outputs']);stages.push({stage:i,i_fields:inputs[0],null_i_fields:inputs[1],fields:outputs[0],null_fields:outputs[1]});}
